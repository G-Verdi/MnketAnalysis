function  mnCHR_plot_map_spm_checkreg_global_max(options, fileName, ...
    doSmooth, threshSignificantF, maxStat, threshContourF, fileUnsmoothedForContour)
...
if doUseUnsmoothedMapForContour
    ...
    fileMaskUnsmoothedForContour = spm_file(fileUnsmoothedForContour, 'prefix', 'peakFWEMask_');
...
% use next neighbor interpolation instead of linear interpolation depiction
if ~doSmooth
...
if doPlotContour
    ...
    % plot params
    linewidth = options.fig.linewidth;
    nblines = 1;
    linestyle = options.fig.linestyle;
