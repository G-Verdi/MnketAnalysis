function D = mnket_erp_m(id, options, doPlot)
...
try
    ...
    %-- By Milad S.: Comparing model-based trials to roving---------------%
    % condlist_roving = mnket_roving_conditions(tones);
    % condlist_roving = mnket_correct_conditions_for_eyeblinktrials(condlist_roving, details.trialstats);
    % rov_list = {strcmp(condlist_roving,'standard');
    %             strcmp(condlist_roving,'deviant')};
    % model_list = {strcmp(condlist,'low');
    %               strcmp(condlist,'high')};
    % similarity_ratio = [length(find((rov_list{1})&(model_list{1})))/length(find(rov_list{1}));
    %                    length(find((rov_list{2})&(model_list{2})))/length(find(rov_list{2}))];
    %  fprintf('\nSimilarity between standard %s and %s = %f\n', 'roving',options.erp.type, num2str(similarity_ratio(1)))
    %  fprintf('\nSimilarity between deviant %s and %s = %f\n', 'roving',options.erp.type, num2str(similarity_ratio(2)))
