        ...
        for c = 1:numel(options.subjects.condition_labels)
            ...
            for s = 1:numel(temp)
                if c == 1  
                    ...
                    options.workdir = fullfile('C:\Users\Gabrielle\Dropbox\Cognemo\MMN\data\prj_epsi\test_mnket');
                ...
                elseif c==2
                    options.workdir = fullfile('C:\Users\Gabrielle\Dropbox\Cognemo\MMN\data\prj_epsi\test_mnpsi');
                    details = mn_subjects(temp{s}, options);
                    % TODO: should be variable when source analysis is used and for ERP analysis
                    icell(idx).scans{s, 1} = fullfile(details.statroot);
