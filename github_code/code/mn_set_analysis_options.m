...
switch uid(1: end-1)     
    case 'desktop-f7jj41h\gabrielle'
        options.preprocdir ='C:\Users\Gabrielle\Dropbox\Cognemo\MMN\data\prj_epsi';
        options.maindir ='C:\Users\Gabrielle\Dropbox\Cognemo\MMN\data';
    otherwise
        error(['Undefined user. Please specify a user in mn_set_analysis_options ' ...
            'and provide the path to the data']);
end
options.workdir = fullfile(options.preprocdir,'group_analysis');
options.rawdir  = fullfile(options.maindir, 'raw');
options.codedir = 'C:\Users\Gabrielle\Documents\Cognemo\code\mnketAnalysis';
options.analysis = 'group_analysis';% analysis type
